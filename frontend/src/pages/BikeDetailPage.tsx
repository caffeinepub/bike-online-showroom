...
export default function BikeDetailPage() {
  ...
  const prevPhoto = () => setActivePhotoIndex((i) => (i - 1 + photos.length) % photos.length);
  ...
  if (isError || !bike) {
    return (
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
        ...
        <h2 className="font-display font-700 text-2xl uppercase text-foreground mb-2">Bike Not Found</h2>
        <p className="text-muted-foreground font-body mb-6">This bike may no longer be available.</p>
        <Button
          onClick={() => navigate({ to: '/' })}
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-12">
          ...
          <div>
            ...
            <div className="relative aspect-[4/3] bg-muted rounded-sm overflow-hidden mb-3 group">
              <img
                ...
                alt={`${bike.name} - photo ${activePhotoIndex + 1}`}
                    ...
                    {photos.map((_, i) => (
                      <button
                        ...
                        className={`w-2 h-2 rounded-full transition-colors ${
                          i === activePhotoIndex ? 'bg-primary' : 'bg-foreground/40'
                        }`}
                ...
                {photos.map((photo, i) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-20 h-16 rounded-sm overflow-hidden border-2 transition-colors ${
                      i === activePhotoIndex ? 'border-primary' : 'border-border hover:border-muted-foreground'
                    }`}
                  >
                    <img
                      src={photo}
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              {bike.engine && (
                <Badge variant="outline" className="border-border font-display font-600 tracking-wider uppercase text-xs rounded-sm">
            ...
            <div className="mb-6">
              <p className="text-xs text-muted-foreground font-body uppercase tracking-wider mb-1">Starting Price</p>
              <p className="font-display font-900 text-4xl text-primary">{formatPrice(bike.price)}</p>
            </div>
        ...
        {/* Specifications */}
        {(bike.engine || bike.weight || bike.frame) && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {bike.engine && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-display font-600 text-xs tracking-widest uppercase text-muted-foreground mb-1">Engine</p>
                      <p className="font-body text-foreground font-medium">{bike.engine}</p>
                    </div>
                  </div>
                )}
                {bike.weight && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-display font-600 text-xs tracking-widest uppercase text-muted-foreground mb-1">Weight</p>
                      <p className="font-body text-foreground font-medium">{Number(bike.weight)} kg</p>
                    </div>
                  </div>
                )}
                {bike.frame && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-display font-600 text-xs tracking-widest uppercase text-muted-foreground mb-1">Frame</p>
                      <p className="font-body text-foreground font-medium">{bike.frame}</p>
                    </div>
                  </div>
                )}
              </div>
