...
import { Loader2, Send, Phone, Mail, User, MessageSquare, Bike, AlertCircle } from 'lucide-react';
    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-card border-b border-border">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <p className="font-display text-xs tracking-[0.3em] uppercase text-primary mb-3">Get In Touch</p>
          ...
          <p className="text-muted-foreground font-body max-w-lg">
            Fill out the form below and our team will get back to you within 24 hours with pricing, availability, and test ride options.
          </p>
        </div>
      </div>
              ...
              <div className="bg-card border border-border rounded-sm p-6 sm:p-8 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="name" className="font-display text-xs tracking-widest uppercase text-muted-foreground flex items-center gap-1.5">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="email" className="font-display text-xs tracking-widest uppercase text-muted-foreground flex items-center gap-1.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="phone" className="font-display text-xs tracking-widest uppercase text-muted-foreground flex items-center gap-1.5">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="bike" className="font-display text-xs tracking-widest uppercase text-muted-foreground flex items-center gap-1.5">
                    ...
                    >
                      ...
                      <SelectContent className="bg-popover border-border rounded-sm">
                        {bikes?.map((bike) => (
                          <SelectItem key={bike.id.toString()} value={bike.id.toString()} className="font-body">
                            {bike.brand} {bike.name}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    {errors.bikeId && <p className="text-destructive text-xs font-body">{errors.bikeId}</p>}
                  </div>
                </div>

                {/* Message */}
                <div className="space-y-1.5">
                  <Label htmlFor="message" className="font-display text-xs tracking-widest uppercase text-muted-foreground flex items-center gap-1.5">
          ...
          <div className="space-y-6">
            <div className="bg-card border border-border rounded-sm p-6">
              ...
              <ol className="space-y-4">
                {[
                  { step: '01', text: 'We receive your inquiry and review your requirements.' },
                  { step: '02', text: 'Our team contacts you within 24 hours.' },
                  { step: '03', text: 'We arrange a test ride or virtual walkthrough.' },
                  { step: '04', text: 'Finalize your purchase with our experts.' },
                ].map(({ step, text }) => (
                  <li key={step} className="flex items-start gap-3">
                    <span className="font-display font-800 text-primary text-sm flex-shrink-0">{step}</span>
            ...
            <div className="bg-primary/10 border border-primary/20 rounded-sm p-6">
              ...
              <div className="flex items-center gap-2 text-primary">
                ...
                <span className="font-display font-600 text-sm tracking-wider">+1 (800) MOTO-VERSE</span>
              </div>
            </div>
          </div>
        </div>
