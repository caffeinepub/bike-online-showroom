...
actor {
  ...
  stable var bikes = Map.empty<Nat, Bike>();
  ...
  // Seed the 20 initial bikes if not already done
  public shared ({ caller }) func seedBikes() : async () {
    if (not seeded) {
      let seededBikes : [(Nat, Bike)] = [
        (
          1,
          {
            id = 1;
      ...
      bikes := Map.fromIter<Nat, Bike>(seededBikes.values());
      seeded := true;
    };
  };
  ...
  // Get a list of all bikes sorted by price
  ...
  // Get all inquiries (possibly for admin use)
